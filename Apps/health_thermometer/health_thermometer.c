...
static int  health_thermometer_writeCb(LEGATTDB_ENTRY_HDR *p);

extern int  blether_writeCb(LEGATTDB_ENTRY_HDR *p);
extern void blether_mi_IndicationConf(void);
...
void health_thermometer_create(void)
{
    ...
    legattdb_deRegWriteHandleCb((LEGATTDB_WRITE_CB)blether_writeCb);
    legattdb_regWriteHandleCb((LEGATTDB_WRITE_CB)health_thermometer_writeCb);
...
int health_thermometer_writeCb(LEGATTDB_ENTRY_HDR *p)
{
    ...
    else if (therAppState->blether_mi_hdl && handle == therAppState->blether_mi_hdl)
    {
        ...
        BLEPROFILE_DB_PDU db_vr_pdu;
        UINT16 written_mi, max_mi, min_mi;

        bleprofile_ReadHandle(therAppState->blether_mi_hdl, &db_pdu);
        ble_tracen((char *)db_pdu.pdu, db_pdu.len);

        written_mi = db_pdu.pdu[0] + (db_pdu.pdu[1]<<8);
        if( therAppState->blether_mi_range_hdl && bleprofile_ReadHandle(therAppState->blether_mi_range_hdl, &db_vr_pdu) == 0 )
        {
            ble_tracen((char *)db_vr_pdu.pdu, db_vr_pdu.len);

            min_mi = db_vr_pdu.pdu[0] + (db_vr_pdu.pdu[1]<<8);
            max_mi = db_vr_pdu.pdu[2] + (db_vr_pdu.pdu[3]<<8);

            if( (written_mi < min_mi) || (written_mi > max_mi) )
            {
                return LEATT_ERR_CODE_OUT_OF_RANGE;
            }
        }
        therAppState->blether_mi = written_mi;
