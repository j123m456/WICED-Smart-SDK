...
*
* To demonstrate the app, work through the following steps (see Secure
* Over the Air Firmware Upgrade Application note for details).
* 1. Plug the WICED eval board into your computer
* 2. Generate rsa_pub.c using WsRsaKeyGen and copy to the project directory
...
* 5. Pair with a client
* 6. Use WsSecOtaUpgrade application to try over the air upgrade
*    ota_firmware_upgrade-BCM920736TAG_Q32-rom-ram-Wiced-release.ota.bin.signed
...
const BLE_PROFILE_CFG hello_sensor_cfg =
{
    ...
    /*.local_name                     =*/ "Hello SecUpgrade",        // [LOCAL_NAME_LEN_MAX];
...
int hello_sensor_write_handler(LEGATTDB_ENTRY_HDR *p)
{
...
#if  0 // following check will allow only paired devices to perform the upgrade
    ...
    {
        ble_trace1("hello_sensor_write_handler: handle %04x", handle);
    }
    ...
    if ((len == 2) && (handle == HANDLE_HELLO_SENSOR_CLIENT_CONFIGURATION_DESCRIPTOR))
    {
        ...
        ble_trace1("hello_sensor_write_handler: client_configuration %04x", hello_sensor_hostinfo.characteristic_client_configuration);

        // Save update to NVRAM.  Client does not need to set it on every connection.
        writtenbyte = bleprofile_WriteNVRAM(NVRAM_ID_HOST_LIST, sizeof(hello_sensor_hostinfo), (UINT8 *)&hello_sensor_hostinfo);
        ble_trace1("hello_sensor_write_handler: NVRAM write:%04x", writtenbyte);
    }
    ...
    {
        ble_trace2("hello_sensor_write_handler: bad write len:%d handle:0x%x", len, handle);
    	return 0x80;
    }
    return 0;
}
...
void hello_sensor_interrupt_handler(UINT8 value)
{
    ...
    ble_trace3("(INT)But1:%d But2:%d But3:%d", value&0x01, (value& 0x02) >> 1, (value & 0x04) >> 2);
